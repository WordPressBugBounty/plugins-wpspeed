...
class Cache implements CacheInterface {
	...
	public static function gc() {
		...
		foreach ( $files as $file ) {
			...
			if ($preserveImages && strpos ( $file, rtrim ( WPSPEED_CACHE_DIR, '/\\' ) . '/images/' ) === 0) {
				continue;
			}
	...
	public static function deleteCache($context = 'both') {
		...
		foreach ( $cache_dir_list as $entry ) {
			...
			if ($preserveImages && $entry ['name'] == 'wpspeed') {
				// Delete everything under wpspeed/ except "images"
				$wpspeed_list = $wp_filesystem->dirlist($cache_dir . DIRECTORY_SEPARATOR . $entry ['name'], false, false);
				
				foreach ($wpspeed_list as $sub) {
					if ($sub['name'] === 'images') {
				...
				if ($entry ['name'] == 'wpspeed' || ($entry ['name'] != 'wpspeed' && $entry ['type'] == 'd' && $deleteAllCacheFolders)) {
					// Normal behaviour: delete the whole wpspeed cache folder
	...
	protected static function getLifetime($page_cache = false) {
		...
		if ($page_cache) {
			if (! $page_cache_lifetime) {
				$params = Plugin::getPluginParams ();
				$page_cache_lifetime = $params->get ( 'page_cache_lifetime', '43200' );
			}
